...
import {
  ...
  IRouterClient,
  QueryResponse,
} from "@finsemble/finsemble-core/types/clients/routerClient";
...
export class BloombergBridgeClient {
   ...
   * ```
   */
  constructor(_routerClient: IRouterClient, logger?: ICentralLogger) {
    if (_routerClient) {
      this.routerClient = _routerClient;
    } else if (FSBL) {
      this.routerClient = FSBL.Clients.RouterClient;
    } else {
      throw new Error(
        "No RouterClient was passed to the constructor and FSBL.Clients.RouterClient was not found!"
      );
    }
    if (logger) {
      this.logger = logger;
      ...
      throw new Error(
        "No Finsemble Logger client was passed to the constructor and FSBL.Clients.Logger was not found!"
      );
    }
  }
  ...
  setEnabled(enabled: boolean, cb?: StandardCallback) {
    ...
    const timeout = setTimeout(() => {
      ...
      if (cb) {
        cb("Set enabled timeout", null);
      }
    }, SET_CONNECT_STATE_TIMEOUT);

    void this.routerClient?.query(
      "BBG_connect",
      { connect: enabled },
      (err, resp: { data?: { status: boolean; message: string } }) => {
        ...
        if (err) {
          ...
          if (resp && resp.data && resp.data["status"]) {
            ...
            console.log(
              "Received negative or empty response when setting enabled: ",
              resp
            );
            if (cb) {
              cb(
  ...
  checkConnection(
    ...
    const timeout = setTimeout(() => {
      console.error(
        "BBG_connection_status check timed-out. Is the Bloomberg Bridge running?",
        null
      );
    ...
    void this.routerClient?.query(
      "BBG_connection_status",
      {},
      (err, resp: { data?: { registered: boolean } }) => {
        clearTimeout(timeout);
        if (err) {
          console.warn(
            "Received error when checking connection status. Is the Bloomberg Bridge running?",
            err
          );
          cb(err, false);
        } else {
          if (resp && resp.data && resp.data["registered"]) {
            ...
            console.debug(
              "Received negative or empty response when checking connection status: ",
              resp
            );
            cb(null, false);
          }
  ...
  queryBloombergBridge(
    ...
    void this.routerClient?.query(
      "BBG_run_terminal_function",
      message,
      this.apiResponseHandler(cb)
    );
  }
  ...
  apiResponseHandler(cb: StandardCallback) {
    return (
      err: StandardError,
      response: QueryResponse<{ status: boolean; message?: string }>
    ) => {
      if (err) {
        ...
        const errMsg = response?.data?.message
          ...
          : "Negative status returned by BBG_run_terminal_function: ";
  ...
  runCreateWorksheet(
    ...
    cb: (
      err: StandardError,
      response: { status: boolean; worksheet: BBGWorksheet }
    ) => void
  ) {
    const message = {
  ...
  runGetAllGroups(
    cb: (
      err: StandardError,
      response: { status: boolean; groups: BBGGroup[] }
    ) => void
  ) {
    const message = {
  ...
  runGetGroupContext(
    ...
    cb: (
      err: StandardError,
      response: { status: boolean; group: BBGGroup }
    ) => void
  ) {
    const message = {
  ...
  runSecurityLookup(
    ...
    cb: (
      err: StandardError,
      response: { status: boolean; results: [{ name: string; type: string }] }
    ) => void
  ) {
