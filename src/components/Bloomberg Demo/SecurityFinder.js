	...
	handleSetGroupContext() {
		if (this.state.isConnected) {
			...
			if (this.state.launchPadGroup && this.state.value) {
				FSBL.Clients.BloombergBridgeClient.runSetGroupContext(
		...
		fdc3.addContextListener((context) => {
			if (context.type === "fdc3.instrument") {
				...
				this.onSuggestionsFetchRequested({ value: data, reason: "input-changed" }, () => {
					if (this.state.finsembleToLaunchpad && this.state.suggestions && this.state.suggestions[0]) {
					...
					if (
						...
						FSBL.Clients.BloombergBridgeClient.runReplaceWorksheet(worksheetId, securities, (err, data) => {
							if (err) {
								...
								this.setState({
									worksheetError: `Error replacing worksheet securities: ${JSON.stringify(err)}`,
									worksheetSuccess: "",
								});
							} else {
						...
						this.setState({
							worksheetError: `Error retrieving worksheet: ${JSON.stringify(err)}`,
							worksheetSuccess: "",
						});
						this.getWorksheets();
					} else {
				...
				<div id="tools" className="flex-dont-grow flex-dont-shrink">
					<Tabs onSelect={this.handleTabSelected}>
						...
						<TabPanel>
							<div className="controlsColumn">
								<div className="controlsRow">
									<label className="inputLabel flex-dont-grow">LaunchPad Group:&nbsp;</label>
									<Select
										name="launchPadGroup"
								...
								<div className="controlsRow justify-flex-start">
									<button className="button" id="setGroupContextButton" onClick={this.handleSetGroupContext}>
										set context
									</button>
									&nbsp;
						...
						<TabPanel>
							<div className="controlsColumn">
								<div className="controlsRow">
									...
									<label className="inputLabel flex-dont-grow">Tails:&nbsp;</label>
									<input
										name="commandTails"
								...
								<div className="controlsRow justify-flex-start">
									<button className="button" id="runCommandButton" onClick={this.handleCommandExecute}>
										execute
									</button>
									&nbsp;
						...
						<TabPanel>
							<div className="controlsColumn">
								<div className="controlsRow">
									<label className="inputLabel flex-dont-grow">Worksheet:&nbsp;</label>
									<Select
										name="worksheetName"
										options={worksheets}
										className="react-select-container"
									...
									/>
									<button className="button flex-dont-grow" id="refreshWorksheetsButton" onClick={this.getWorksheets}>
										refresh
									</button>
								</div>
								<div className="controlsRow justify-flex-start">
									<button className="button" id="addToWorksheetButton" onClick={this.handleAddToWorksheet}>
										Add to worksheet
									</button>
									&nbsp;
