      ...
      (err, response) => {
        if (err) {
          FSBL.Clients.Logger.error(
            `Error received from runBBGCommand: mnemonic: ${mnemonic}, securities: ${JSON.stringify(
              securities
            )}, panel: ${panel}, tails: ${tails}, error:`,
            err
          );
...
window.createWorksheet = () => {
  ...
  if (!error) {
    bbg.runCreateWorksheet(worksheetName, securities, (err, data) => {
      if (err) {
        FSBL.Clients.Logger.error(
          `Error received from runCreateWorksheet: worksheetName: ${worksheetName}, securities: ${JSON.stringify(
            securities
          )}, error:`,
          err
        );
...
window.getAllWorksheets = () => {
  ...
  bbg.runGetAllWorksheets((err, response) => {
    if (response && response.worksheets && Array.isArray(response.worksheets)) {
      ...
      FSBL.Clients.Logger.error(
        "Error received from runGetAllWorksheets:",
        err
      );
      showElement("allWorksheetsError");
    } else {
      FSBL.Clients.Logger.error(
        "invalid response from runGetAllWorksheets",
        response
      );
      showElement("allWorksheetsError");
    }
  ...
  bbg.runGetWorksheet(worksheetId, (err, response) => {
    ...
    if (
      ...
      renderWorksheet(
        ...
        response.worksheet.id,
        response.worksheet.securities
      );
    } else if (err) {
      FSBL.Clients.Logger.error(
        `Error received from runGetWorksheet: worksheetId: ${worksheetId}, error:`,
        err
      );
      showElement("worksheetError");
    } else {
...
window.replaceWorksheet = () => {
  ...
  if (!error) {
    bbg.runReplaceWorksheet(worksheetId, securities, (err, data) => {
      if (err) {
        FSBL.Clients.Logger.error(
          "Error received from runReplaceWorksheet:",
          err
        );
        showElement("worksheetError");
      } else {
...
window.getAllGroups = () => {
  ...
  bbg.runGetAllGroups((err, response) => {
    if (response && response.groups && Array.isArray(response.groups)) {
      ...
      FSBL.Clients.Logger.error(
        "invalid response from runGetAllGroups",
        response
      );
      showElement("allGroupsError");
    }
...
window.toggleGroup = (elementId, group) => {
  ...
  if (detailsArr.length > 0) {
    Array.prototype.forEach.call(detailsArr, (deets) => {
      deets.remove();
    });
  } else {
...
window.setGroupContext = (detailsElement, sector) => {
  ...
  bbg.runSetGroupContext(name, newValue, null, (err, data) => {
    if (err) {
      FSBL.Clients.Logger.error(
        `Error received from runSetGroupContext, group: ${name}, value: ${newValue}, error: `,
        err
      );
...
window.refreshGroupContext = (detailsElement) => {
  ...
  bbg.runGetGroupContext(groupName, (err, data2) => {
    if (err) {
      FSBL.Clients.Logger.error(
        `Error received from runGetGroupContext: groupName: ${groupName}, error:`,
        err
      );
...
window.runSecurityLookup = () => {
  ...
  if (!error) {
    ...
    bbg.runSecurityLookup(security, (err, data) => {
      if (err) {
        FSBL.Clients.Logger.error(
          `Error received from runSecurityLookup: search string: ${security}, error: `,
          err
        );
        ...
        successLabel.innerHTML =
          "Security Lookup successful, response time: " +
          (end - start) +
          " milliseconds<br>" +
          "results:<br>" +
          results;
...
window.displayCol = (elementName) => {
  displayType(
    document.getElementById(elementName + "Heading"),
    document.getElementById(elementName + "Col")
  );
